...
def test_globaljj_sparse_fit_matches_dense():
    X, y = _make_sparse_binary_case(seed=0)
    ...
    np.testing.assert_allclose(
        sparse_state.family_state.xi,
        dense_state.family_state.xi,
        rtol=1e-5,
        atol=1e-5,
    )

def test_globaljj_mean_message_sparse_smoke():
    X, y = _make_sparse_binary_case(seed=4)
    Xs = sparse.BCOO.fromdense(jnp.asarray(X))
    data = prep_data(Xs, y)
